<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl text-white leading-tight">Editar Registro de Alimentación</h2>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-xl">
                <div class="p-6">
                    <form method="POST" action="{{ route('alimentacion.update', $alimentacion) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <select id="lote_id" name="lote_id"
                                ...
                                @foreach($lotes as $lote)
                                    <option value="{{ $lote->id }}"
                                        {{ old('lote_id', $alimentacion->lote_id) == $lote->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" id="tipo_alimento" name="tipo_alimento"
                                   value="{{ old('tipo_alimento', $alimentacion->tipo_alimento) }}"
                                   class="w-full border-gray-300 focus:border-[#3b4a67] focus:ring-[#3b4a67] rounded-md shadow-sm">
                            @error('tipo_alimento')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-5">
                            <div>
                                ...
                                <input type="number" id="cantidad_kg" name="cantidad_kg"
                                       value="{{ old('cantidad_kg', $alimentacion->cantidad_kg) }}"
                                       step="0.01" min="0.1"
                                       class="w-full border-gray-300 focus:border-[#3b4a67] focus:ring-[#3b4a67] rounded-md shadow-sm">
                                @error('cantidad_kg')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <input type="date" id="fecha" name="fecha"
                                       value="{{ old('fecha', $alimentacion->fecha) }}"
                                       class="w-full border-gray-300 focus:border-[#3b4a67] focus:ring-[#3b4a67] rounded-md shadow-sm">
                                @error('fecha')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror

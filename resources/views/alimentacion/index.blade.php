    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <h2 class="font-semibold text-xl text-white leading-tight">Alimentación</h2>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-[#3b4a67]">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold text-white uppercase tracking-wider">Tipo Alimento</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-white uppercase tracking-wider">Cantidad (kg)</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400 font-mono">{{ $alimentacion->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                Lote #{{ $alimentacion->lote->id }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $alimentacion->tipo_alimento }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                {{ number_format($alimentacion->cantidad_kg, 2) }} kg
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $alimentacion->fecha }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm flex items-center gap-4">
                                ...
                                <form action="{{ route('alimentacion.destroy', $alimentacion) }}" method="POST"

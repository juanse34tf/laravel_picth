<aside class="fixed top-0 left-0 h-full w-64 bg-[#3b4a67] flex flex-col z-50 overflow-y-auto">
    ...
    <div class="px-3 py-4 border-t border-[#2f3c55] space-y-0.5">
        <a href="{{ route('profile.edit') }}"
           class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-300 hover:bg-[#2f3c55] hover:text-white transition-colors">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                class="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-300 hover:bg-red-700 hover:text-white transition-colors">

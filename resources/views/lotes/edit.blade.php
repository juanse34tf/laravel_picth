<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar Lote</h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('lotes.update', $lote) }}">
                        ...
                        <div class="mb-5">
                            ...
                            <input type="date" id="fecha_inicio" name="fecha_inicio"
                                   value="{{ old('fecha_inicio', $lote->fecha_inicio) }}"
                                   class="w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            @error('fecha_inicio')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-6">
                            ...
                            <select id="estado" name="estado"
                                ...
                                <option value="Finalizado" {{ old('estado', $lote->estado) === 'Finalizado' ? 'selected' : '' }}>Finalizado</option>
                                <option value="En Espera"  {{ old('estado', $lote->estado) === 'En Espera'  ? 'selected' : '' }}>En Espera</option>
                            </select>
                            @error('estado')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror

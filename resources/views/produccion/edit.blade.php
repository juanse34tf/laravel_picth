<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar Registro de Producción</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('produccion.update', $produccion) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-5">
                            ...
                            <div>
                                ...
                                <input type="text" id="tipo_huevo" name="tipo_huevo"
                                       value="{{ old('tipo_huevo', $produccion->tipo_huevo) }}"
                                       class="w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                @error('tipo_huevo')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="observaciones" name="observaciones" rows="3"
                                      class="w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('observaciones', $produccion->observaciones) }}</textarea>
                            @error('observaciones')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
